...
use cli::{
	commands::{
		agent_host, agent_kill, agent_logs, agent_proxy, agent_ps, agent_stop, args, serve_web,
		tunnels, update, version, CommandContext,
	},
	...
	let result = match parsed {
		...
		| args::AnyCli::Integrated(args::IntegratedCli { core: c, .. }) => match c.subcommand {
			...
			Some(args::Commands::Agent(agent_args)) => match agent_args.subcommand {
				...
				Some(args::AgentSubcommand::Proxy(proxy_args)) => match proxy_args.subcommand {
					args::AgentProxySubcommand::Claude(claude_args) => {
						agent_proxy::agent_proxy_claude(context!(), claude_args).await
					}
				},
